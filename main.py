...
print("RUNNING VERSION: structured-prizes-9-inactivity-rate-limit-fix")
...
def get_show_tag_line(show: str | None) -> str:
    ...
    if normalized in HAPPY_HOUR_ALIASES:
        return "Tag **Koach**."
    if normalized in LONDON_ALIASES:
        return "Tag **Luca**."
    return "Tag **Max** and **Lama**."
...
async def _inactivity_loop():
    ...
    while not bot.is_closed():
        try:
            ...
            for row in rows:
                # 1 second delay between each ticket check to stay well under
                # Discord's 50 req/sec global rate limit when many tickets are open.
...
class FaqCategoryDeleteSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not category:
            ...
            return
        view = FaqCategoryDeleteConfirmView(category_id=category_id, name=category["name"])
        await interaction.response.edit_message(
            content=(
                f"⚠️ **Confirm delete category**\n\n"
                f"**Category:** {category['name']}\n\n"
                "This will soft-delete the category and all its entries. They can be restored via the database."
            ),
            view=view
        )


class FaqCategoryDeleteView(discord.ui.View):
    def __init__(self, categories: list[dict]):
        super().__init__(timeout=120)
        self.add_item(FaqCategoryDeleteSelect(categories))


class FaqCategoryDeleteConfirmView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            with get_db() as conn:
                ...
                conn.commit()
            await interaction.response.edit_message(
                content=f"🗑️ Category **{self.name}** and all its entries have been deleted.",
                view=None
            )
        except Exception as e:
            await interaction.response.edit_message(
                content=f"❌ Failed to delete category: `{e}`",
                view=None
            )
...
@faq_category_group.command(name="delete", description="Delete a FAQ category and all its entries (soft delete)")
async def faq_category_delete(interaction: discord.Interaction):
    if not await ensure_mod(interaction):
        return
    categories = get_active_faq_categories()
    if not categories:
        await interaction.response.send_message("❌ No active categories found.", ephemeral=True)
        return
    view = FaqCategoryDeleteView(categories)
    await interaction.response.send_message(
